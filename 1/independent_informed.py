...
from dependent_informed import get_children #import get_children function

from queue import PriorityQueue #import the queue that we use

# this function runs the uniform cost search
def search(g, s, c, start, goal, heuristics_cask_goal, heuristics_no_cask_goal):
    path=''#initialize the path string
    explored = set() #initialize the set that will contain the explored states
    queue = PriorityQueue() #initialize queue
    queue.put((0, start, path, 0)) #initialize queue
    count = 0#initialize the nodes counter

    # this while runs until that is nothing in the queue or reaches a break/return
    while queue:

        cost_heur, state, path, cost = queue.get()#queue.get() picks the state with the minimum cost + heuristic
                                                  #  and pops it out of the queue
        ...
        if state not in explored:
            ...
            if state[0:7] == goal:
                return [path, cost_heur, cost, count] #return values


            # generate children from the current state
            queue = get_children(g, s, c, explored, cost_heur, path, state, queue, goal, cost, heuristics_cask_goal, heuristics_no_cask_goal)
